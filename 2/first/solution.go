...
func main() {
	...
	for _, input := range inputs {
		games = append(games, createGame(input))
	}

	var res int = 0
	for _, game := range games {
		var isValid bool = isValidGame(game)
		if isValid {
			fmt.Println(strconv.Itoa(game.id), " is valid")
			res += game.id
		}
	}
...
func isValidGame(game Game) bool {
	return game.nRed <= maxRed && game.nBlue <= maxBlue && game.nGreen <= maxGreen
}

func createGame(input string) Game {
	split := strings.Split(input, ":")
	var id int = getId(split[0])
	var newGame Game = Game{id, 0, 0, 0}

	roundStrs := strings.Split(split[1], ";")

	for _, roundStr := range roundStrs {
		newRound := createRound(roundStr)
		if newRound.nRed > newGame.nRed {
			newGame.nRed = newRound.nRed
		}
		if newRound.nBlue > newGame.nBlue {
			newGame.nBlue = newRound.nBlue
		}
		if newRound.nGreen > newGame.nGreen {
			newGame.nGreen = newRound.nGreen
		}
	}
...
func createRound(input string) Round {

	newRound := Round{0, 0, 0}

	firstSplit := strings.Split(input, ",")
	for _, segment := range firstSplit {
		if strings.Contains(segment, "blue") {
			segment = strings.Replace(segment, "blue", "", 1)
			...
			newRound.nBlue = num
		} else if strings.Contains(segment, "red") {
			segment = strings.Replace(segment, "red", "", 1)
			segment = strings.Replace(segment, " ", "", -1)
			num, err := strconv.Atoi(segment)
			if err != nil {
				fmt.Println("Error:", err)
			...
			newRound.nRed = num
		} else if strings.Contains(segment, "green") {
			segment = strings.Replace(segment, "green", "", 1)
			segment = strings.Replace(segment, " ", "", -1)
			num, err := strconv.Atoi(segment)
			if err != nil {
				fmt.Println("Error:", err)
