...
func main() {
	...
	// CREATE MASK AND STARTING POINT 'S'
	mask, sCoordinates := createMask(rows)

	// FIND LOOP AND COUNT STEPS
	var afterCoordinates Coordinates = getFirstCoordinates(mask, sCoordinates)

	for {
		newCoordinates, err := calculateNewCoords(mask, sCoordinates, afterCoordinates)
		if err != nil {
			syscall.Exit(1)
		}
		*mask[afterCoordinates.row][afterCoordinates.col] = 'S'
		if checkValidCoords(mask, newCoordinates) && *mask[newCoordinates.row][newCoordinates.col] == 'S' {
			break
		}
		sCoordinates = afterCoordinates
		afterCoordinates = newCoordinates
	}

	// CALCULATE IN AREA
	*mask[0][0] = '0'
	calculateInArea(mask, Coordinates{0, 0})
...
func createMask(rows []string) (mask [][]*rune, s Coordinates) {
	for i := 0; i < (2*len(rows))+1; i++ {
		var newRow []*rune
		for k := 0; k < (2*len(rows))+1; k++ {
			if i%2 == 0 {
				newRune := '|'
				newRow = append(newRow, &(newRune))
			} else {
				if k%2 == 0 {
					newRune := '-'
					newRow = append(newRow, &(newRune))
				} else {
					cell := rune(rows[i/2][k/2])
					if cell == 'S' {
						s = Coordinates{i, k}
					}
...
func calculateInArea(mask [][]*rune, coords Coordinates) {
	for i := coords.row - 1; i <= coords.row+1; i++ {
		for k := coords.col - 1; k <= coords.col+1; k++ {
			...
			if *mask[i][k] != 'S' && *mask[i][k] != '0' {
				*mask[i][k] = '0'
				calculateInArea(mask, Coordinates{i, k})
			}
		}
	}
}

func getFirstCoordinates(mask [][]*rune, prevCoords Coordinates) Coordinates {
	afterCoordinates2 := Coordinates{prevCoords.row + 1, prevCoords.col + 0}
	doubleAfterCoordinates := Coordinates{prevCoords.row + 2, prevCoords.col + 0}
	_, err := calculateNewCoords(mask, afterCoordinates2, doubleAfterCoordinates)
	if err == nil {
		return afterCoordinates2
	}
	afterCoordinates2 = Coordinates{prevCoords.row - 1, prevCoords.col + 0}
	doubleAfterCoordinates = Coordinates{prevCoords.row - 2, prevCoords.col + 0}
	_, err = calculateNewCoords(mask, afterCoordinates2, doubleAfterCoordinates)
	...
	afterCoordinates2 = Coordinates{prevCoords.row + 0, prevCoords.col + 1}
	doubleAfterCoordinates = Coordinates{prevCoords.row + 0, prevCoords.col + 2}
	if err == nil {
		return afterCoordinates2
	}
	afterCoordinates2 = Coordinates{prevCoords.row + 0, prevCoords.col - 1}
	...
	fmt.Println("Impossible to find first coords!")
	syscall.Exit(1)
	return afterCoordinates2
}

func calculateNewCoords(mask [][]*rune, prev Coordinates, curr Coordinates) (after Coordinates, err error) {
	if !checkValidCoords(mask, curr) {
		fmt.Errorf("Not valid coordinates %d, %d", curr.row, curr.col)
		err = &CalcError{"Not valid coordinates"}
		return after, err
	}

	var pipe rune = *mask[curr.row][curr.col]

	if pipe == 'J' {
		after, err = calculateJ(prev, curr)
	} else if pipe == 'F' {
		after, err = calculateF(prev, curr)
	} else if pipe == 'L' {
		after, err = calculateL(prev, curr)
	} else if pipe == '7' {
		after, err = calculate7(prev, curr)
	} else if pipe == '|' {
		after, err = calculateVertical(prev, curr)
	} else if pipe == '-' {
		after, err = calculateHorizontal(prev, curr)
	} else {
		fmt.Errorf("Pipe not recognized '%c'", pipe)
		err = &CalcError{"Pipe not recognized"}
	}
	return after, err
}

func checkValidCoords(mask [][]*rune, curr Coordinates) bool {
	if curr.row < 0 || curr.row >= len(mask) {
		return false
	} else if curr.col < 0 || curr.col >= len(mask[curr.row]) {
		return false
	}
	return true
}

func calculateJ(prev Coordinates, curr Coordinates) (after Coordinates, err error) {
	if curr.row-prev.row == 0 && curr.col-prev.col == 1 {
		after = Coordinates{curr.row - 1, curr.col}
	} else if curr.row-prev.row == 1 && curr.col-prev.col == 0 {
		after = Coordinates{curr.row, curr.col - 1}
	} else {
		fmt.Errorf("Impossible to calc, %d, %d", curr.row, curr.col)
		err = &CalcError{"Impossible to calc"}
	}
	return after, err
}

func calculateF(prev Coordinates, curr Coordinates) (after Coordinates, err error) {
	if curr.row-prev.row == 0 && curr.col-prev.col == -1 {
		after = Coordinates{curr.row + 1, curr.col}
	} else if curr.row-prev.row == -1 && curr.col-prev.col == 0 {
		after = Coordinates{curr.row, curr.col + 1}
...
func calculateL(prev Coordinates, curr Coordinates) (after Coordinates, err error) {
	if curr.row-prev.row == 0 && curr.col-prev.col == -1 {
		after = Coordinates{curr.row - 1, curr.col}
	} else if curr.row-prev.row == 1 && curr.col-prev.col == 0 {
		after = Coordinates{curr.row, curr.col + 1}
...
func calculate7(prev Coordinates, curr Coordinates) (after Coordinates, err error) {
	if curr.row-prev.row == 0 && curr.col-prev.col == 1 {
		after = Coordinates{curr.row + 1, curr.col}
	} else if curr.row-prev.row == -1 && curr.col-prev.col == 0 {
		after = Coordinates{curr.row, curr.col - 1}
...
func calculateVertical(prev Coordinates, curr Coordinates) (after Coordinates, err error) {
	if curr.row-prev.row == -1 && curr.col-prev.col == 0 {
		after = Coordinates{curr.row - 1, curr.col}
	} else if curr.row-prev.row == 1 && curr.col-prev.col == 0 {
		after = Coordinates{curr.row + 1, curr.col}
	} else {
...
func calculateHorizontal(prev Coordinates, curr Coordinates) (after Coordinates, err error) {
	if curr.row-prev.row == 0 && curr.col-prev.col == 1 {
		after = Coordinates{curr.row, curr.col + 1}
	} else if curr.row-prev.row == 0 && curr.col-prev.col == -1 {
		after = Coordinates{curr.row, curr.col - 1}
	} else {
		fmt.Errorf("Impossible to calc, %d, %d", curr.row, curr.col)
		err = &CalcError{"Impossible to calc"}
...
const inputs = `FFFJJ--JF|JF7-7FJ7FFJ-F--J7F-77.-7-7L|.F.|7F7FLJ-7-|-FFJ-FL7-7FF|-|-7FF7777L-L-7J----JF7.F.F--777FF|77F7|F|777F7LF-J7..-7.777F.F.-.FF.7FF7-7
