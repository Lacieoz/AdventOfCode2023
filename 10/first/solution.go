...
func main() {
	...
	for cond {
		...
		i++
		if checkValidCoords(mask, afterCoordinates) && mask[afterCoordinates.row][afterCoordinates.col] == 'S' {
			break
		}
	}
...
func getFirstCoordinates(mask [][]rune, prevCoords Coordinates) Coordinates {
	afterCoordinates := Coordinates{prevCoords.row + 1, prevCoords.col + 0}
	_, err := calculateNewCoords(mask, prevCoords, afterCoordinates)
	if err == nil {
	...
	afterCoordinates = Coordinates{prevCoords.row + 0, prevCoords.col + 1}
	_, err = calculateNewCoords(mask, prevCoords, afterCoordinates)
	if err == nil {
		return afterCoordinates
	}
